...
def _generate_with_agents(req: GenerateRequest) -> dict[str, Any]:
    ...
    try:
        ...
        # Format images from the workflow (already generated by generator_agent)
        images_result: list[Any] = []
        generated_images = final_state.get("generated_images", [])
        
        if generated_images:
            # Images are stored as base64 strings, convert to the expected format
            ...
            for prompt_idx in range(len(prompts)):
                prompt_images = []
                start_idx = prompt_idx * images_per_prompt
                end_idx = start_idx + images_per_prompt
                
                for img_idx in range(start_idx, min(end_idx, len(generated_images))):
                    if img_idx < len(generated_images):
                        prompt_images.append({
                            "b64_json": generated_images[img_idx],
                            "url": None,
                            "revised_prompt": None
                        })
                
                if prompt_images:
                    images_result.append(prompt_images)
        ...
        return {
            ...
            "prompts": final_state.get("current_prompts", []),
            "images": images_result,
            "generation_ids": final_state.get("generation_ids", []),  # IDs for approval/library saving
        }
